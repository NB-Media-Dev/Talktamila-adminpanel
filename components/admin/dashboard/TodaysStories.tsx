...
export default function TodayStories() {
  ...
  return (
    <div className="w-full max-w-full bg-white rounded-[24px] sm:rounded-[32px] p-3.5 sm:p-5 shadow-[0_4px_24px_rgba(0,0,0,0.03)] border border-[#FFEFE0] overflow-hidden">
    
      <div className="flex items-center justify-between mb-2 py-1.5 sm:py-2">
        <div className="flex items-center gap-2">
          <div className="p-1 sm:p-1.5 rounded-lg bg-red-50 text-red-500">
            <Zap className="w-4 h-4 sm:w-5 sm:h-5 fill-orange-400 stroke-orange-200" />
          </div>
          <h2 className="text-sm sm:text-lg font-bold text-gray-900 tracking-tight">
            Today&apos;s Stories
          </h2>
        </div>
        <button className="text-[11px] sm:text-sm text-[#FF6B35] hover:text-[#D9652B] font-semibold transition-colors cursor-pointer shrink-0">
          View All
        </button>
      </div>

      <div className="flex items-center gap-3 sm:gap-4 overflow-x-auto no-scrollbar pb-1 mb-2 sm:mb-3 w-full max-w-full">
        {stories.map((story) => (
          <div key={story.id} className="flex flex-col items-center gap-1.5 shrink-0 group cursor-pointer">
            ...
            <div className="relative">
              ...
              <div className="p-[2px] rounded-full bg-gradient-to-tr from-[#FF4B2B] via-[#FF416C] to-[#FF6B35] transition-transform duration-300">
                <div className="w-[46px] h-[46px] sm:w-[58px] sm:h-[58px] rounded-full border-2 border-white overflow-hidden relative bg-gray-50">
                  <Image
                    src={story.avatar}
                    alt={story.userName}
                    fill
                    sizes="(max-width: 640px) 46px, 58px"
