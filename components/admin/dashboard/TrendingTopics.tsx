 ...
 UsetimeoutLoader(setIsLoading)
  ...
  return (
    <Cardlayout
      ...
      isLoading={isLoading} 
      skeleton={<ContentSkeleton count={4} height="h-[30px]" width="w-full" />} 
    >
      ...
      <div>
        <div className="flex flex-col mb-4">
          {adminTopics.map((topic, index) => (
            ...
            >
              <span className="text-[#FF6B35]  text-sm font-medium hover:underline cursor-pointer">
                {topic.hashtag}
              </span>
              <span className="text-gray-400 text-sm font-medium">{topic.posts}</span>
            </div>
          ))}
        </div>
